    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->isMethod('post')) {
                $data = $request->all();
                $rules = [
                    'name' => 'required', 'cat_id' => 'required', 'user_id' => 'required', 'image' => 'required|image',
                ...
                $service->create([
                    'name' => $data['name'], 'slug' => $this->CustomeSlug($data['name']), 'cat_id' => $data['cat_id'], 'user_id' => Auth::id(),
                    'image' => $filename, 'description' => $data['description'], 'price' => $data['price'], 'status' => $data['status']
                ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            if ($request->isMethod('post')) {
                ...
                $service->update([
                    'name' => $data['name'], 'slug' => $this->CustomeSlug($data['name']), 'cat_id' => $data['cat_id'],
                    'description' => $data['description'], 'price' => $data['price'], 'status' => $data['status']
                ]);
